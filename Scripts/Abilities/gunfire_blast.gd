...
@export var max_lifetime: float = 0.5
...
func _physics_process(delta: float) -> void:
	...
	if target and is_instance_valid(target) and target.current_state != target.CharacterState.DEAD:
		var dist = global_position.distance_to(target.global_position + Vector3(0, 1.1, 0))
		if dist <= 1.4:
			target.take_damage(damage, "gunfire", direction, 4.5)
